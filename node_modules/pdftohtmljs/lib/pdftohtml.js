...
function pdftohtml(filename, outfile, options) {
    ...
    pdftohtml.prototype.convert = function () {
        var self = this;
        console.log('pdf2htmlEX ' + self.options.additional.join(' ') + "{async:true, silent:true}");
        var child = shell.exec('pdf2htmlEX ' + self.options.additional.join(' ') + " --process-nontext 0 ", {
            async: true,
            silent: true
        });
